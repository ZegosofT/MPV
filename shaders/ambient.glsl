...
// --- CONFIGURATION ---
#define BLUR_SAMPLES 32      // PC can handle 32+ samples easily
...
vec4 hook() {
    ...
    // Dynamically injected inverse scale natively cancels the Lua stretch
    vec2 inv_scale = vec2(1.0000, 1.3187);
    vec2 center_uv = (uv - 0.5) * inv_scale + 0.5;
    
    // 1. Output Sharp Center Video
    if (all(greaterThanEqual(center_uv, vec2(0.0))) && all(lessThanEqual(center_uv, vec2(1.0)))) {
        return HOOKED_tex(center_uv); 
    }
    
    // 2. Generate Anamorphic Blur
